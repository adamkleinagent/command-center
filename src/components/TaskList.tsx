...
export function TaskList({ 
  ...
  const handleCreateTask = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!newTaskTitle.trim() || activeProjectId === 'all') return;

    const { data, error } = await createTask(activeProjectId, newTaskTitle);
    if (data) {
      onTaskCreated(data);
      setNewTaskTitle('');
      setIsCreating(false);
  ...
  return (
    <div className="flex-1 h-screen bg-zinc-950 flex flex-col border-r border-zinc-900">
      ...
      {/* Create Task Inline */}
      {isCreating && (
        <form onSubmit={handleCreateTask} className="mx-8 mt-4">
          <input 
            autoFocus
            type="text"
            value={newTaskTitle}
            ...
            className="w-full bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-3 text-sm text-zinc-100 outline-none focus:border-indigo-500 shadow-2xl transition-all"
